...
for sldfile in *.sld; do
  ...
  # Delete style 
  curl -v -u $login -XDELETE $restapi/workspaces/$workspace/styles/$layername

done

curl -v -u $login -XDELETE http://localhost:8080/geoserver/rest/workspaces/osm/layers/park?recurse=true
